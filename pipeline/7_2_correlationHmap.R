#Comparison of kcl (control sample from previous experiment) with public db
#pearson pairwise correlations between cluster-based pseudobulk of kcl and public bulk data
setwd("/mnt/picea/home/schoudhary/shruti")
set.seed(42)
...
integ<- readRDS("SNRIII/data/SeuratOut/integ.rds")
...
kno <- NormalizeData(kno)
kno <- FindVariableFeatures(kno, selection.method = "vst", nfeatures = 2000)
kno <- ScaleData(kno)
...
kno$oldclusters <- kno$seurat_clusters
kno$seurat_clusters <- kno@active.ident
kno$orig.ident <- paste0("kno_",kno$seurat_clusters)
kno$orig.samp <- "kno"

#pseudobulk
kcl0bulkscale <- AggregateExpression(kcl0, group.by = "orig.ident") #slot = "data"
kno0bulkscale <- AggregateExpression(kno, group.by = "orig.ident") #slot = "data"
#kcl0bulkcount <- AggregateExpression(kcl0, group.by = "orig.ident", slot = "counts")
# 'scale' is normalized count which I'll use. If raw count is needed in any case,
# one can add slot = "counts"

saveRDS(kcl0bulkscale, file = "SNR-u2023011/analysis/kcl0bulkscale.rds")
saveRDS(kno0bulkscale, file = "SNR-u2023011/analysis/kno0bulkscale.rds")

# Clear workspace and restart R
##1. Aspwood
#Load AspWood TPM
#Ref: https://plantgenie.org/FTP?dir=Data%2FPlantGenIE%2FPopulus_tremula%2Fv2.2%2FExpression
aspwood <- read.table("SNR-u2023011/analysis/publisheddatasets/AspWood_tpm.txt", header = TRUE)
...
# Note: go to big heatmap section instead if you want a collective heatmap

# if you want individual hetamap, follow this
...
pheatmap(pcor, clustering_method = "ward.D2", fontsize = 8) 
#Heatmap with aspwood order preserved
pheatmap(pcor, fontsize = 8, cluster_rows = F, cluster_cols = F) 
...
shilcm <- read.table("SNR-u2023011/analysis/publisheddatasets/Tung/GSE81077_Tissue_LCM_rawcount.txt", header = T)
#Normalize
...
#Load ortholog list
potrapotri <- read.table(gzfile("SNR-u2023011/analysis/publisheddatasets/potra_potri_BEST_DIAMOND_out.tsv.gz"))
...
nrow(exp) #25772
pcor <- cor(exp, method = "pearson")
pheatmap(pcor, fontsize = 8)

##3. Tung
#Tung bulk data
#Ref: https://link.springer.com/article/10.1186/s13059-022-02845-1 
tungtpm <- read.table("SNR-u2023011/analysis/publisheddatasets/Tung/GSE180121_Cell_type_TPM_Ptr.txt", sep = "\t", header = T)
tungtpm$Potri <- sub(".v4.1","",tungtpm$Gene.ID)

#Load potra to potri diamond hits
potrapotri <- read.table("SNR-u2023011/analysis/publisheddatasets/potra_potri_BEST_DIAMOND_out.tsv.gz")
colnames(potrapotri) <- c("Potra","Potri")
...
# Big heatmap section: make one big correl heatmap
# aspwood
aspwood <- read.table("SNR-u2023011/analysis/publisheddatasets/AspWood_tpm.txt", header = TRUE)
aspwoodtpm <- dcast(aspwood, gene_id ~ sample_name)

##Potri to potra datasets
potrapotri <- read.table(gzfile("SNR-u2023011/analysis/publisheddatasets/potra_potri_BEST_DIAMOND_out.tsv.gz"))
colnames(potrapotri) <- c("Potra","Potri")

#Tung bulk data
tungtpm <- read.table("SNR-u2023011/analysis/publisheddatasets/Tung/GSE180121_Cell_type_TPM_Ptr.txt", sep = "\t", header = TRUE)
tungtpm$Potri <- sub(".v4.1","",tungtpm$Gene.ID)
tungtpmPotra <- inner_join(tungtpm,potrapotri)
tungtpmPotra$Potri <- NULL
tungtpmPotra$Gene.ID <- NULL
...
shilcm <- read.table("SNR-u2023011/analysis/publisheddatasets/Tung/GSE81077_Tissue_LCM_rawcount.txt", header = TRUE)
rownames(shilcm) <- shilcm$gene
shilcm$gene <- NULL
shilcmmeta <- data.frame(colnames(shilcm),sub(".[123]","",colnames(shilcm)))
colnames(shilcmmeta) <- c("sampleID","celltype")
...
# Conde et al, 2023
conde <- readRDS("SNR-u2023011/analysis/publisheddatasets/condeVascularCells.rds")
...
# If you want all data to be displayed
pseudobulk <- as.data.frame(kcl0bulkscale$RNA)
# pseudobulk <- as.data.frame(kno0bulkscale$RNA)
pseudobulk$Potra <- rownames(pseudobulk)
exp <- inner_join(pseudobulk,aspwooddrop)
...
# pheatmap(pcor, fontsize = 7, cluster_rows = F, display_numbers = T,
#          cluster_cols = F)
saveRDS(pcor, file = "SNR-u2023011/analysis/kcl0PcorAllData.rds")
# Do for kno sample and save
saveRDS(pcor, file = "SNR-u2023011/analysis/kno0PcorAllData.rds")

pcorsubset <- pcor[-grep("kcl*", rownames(pcor)),grep("kcl*", rownames(pcor))]
# pheatmap(pcorsubset, fontsize = 7, cluster_rows = F, cluster_cols = F,
#          display_numbers = round(pcorsubset, digits = 2))

#Set min color range at 0.3/ 0.2
...
pheatmap(pcorsubset, fontsize = 2, cluster_rows = F, #cluster_cols = F,
         ...
         color = magma(100), #other options: inferno, viridis, mako
         border_color = NA, breaks = mat_breaks, 
         gaps_row = c(25,37,61,71,88,95,121,149)) #adjust the gap according to data)